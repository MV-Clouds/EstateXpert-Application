...
export default class EmailCampaignTemplateForm extends LightningElement {
    ...
    handleAddNewEmail() {
        const newId = this.dripEmails.length + 1;
        this.dripEmails = [...this.dripEmails, { id: newId, name: 'New Email', subject: '', daysAfterStartDate: 0, timeToSend: '' }];
    }

    handleDeleteEmail(event) {
        const emailId = event.currentTarget.dataset.id;
        this.dripEmails = this.dripEmails.filter(email => email.id !== parseInt(emailId, 10));
    }
