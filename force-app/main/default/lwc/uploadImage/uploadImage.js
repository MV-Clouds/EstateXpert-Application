...
export default class UploadImage extends LightningElement {
    ...
    @track interior_checked=false;
    @track exterior_checked=false;
    ...
    upload_image() {
        ...
        if (this.imageTitle_to_upload && this.imageUrl_to_upload) {
            ...
            if (this.selected_url_type === 'Image') {
                ...
                // var imgregEx = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif|png|svg))/i
                // var match = this.imageUrl_to_upload.match(regExp);
                if(!this.imageUrl_to_upload.match(/\.(png|jpg|jpeg|gif|png|svg)(\?.*)?$/)){
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Type not valid',
                            message: 'Invalid Url kindly check url and type',
                            variant: 'error',
                        }),
                    );
                    this.ispopup = true;
                ...
                if(!this.imageUrl_to_upload.match(/\.(png|jpg|jpeg|gif|png|svg)(\?.*)?$/)){
                    ...
                    // var match = this.imageUrl_to_upload.match(regExp);
                        ...
                        fileContent.push({
                            ...
                            IsOnPortalFeed: true,
                            IsOnWebsite: true,
                            externalVideoUrl:this.imageUrl_to_upload
                        })
                        createmedia({
                            recordId: this.recordId,
                            mediaList:fileContent
    ...
    edit_image_name_to_store(event) {
        ...
        this.interior_checked=false;
        this.exterior_checked=false;
        ...
        if(list_check.length>0){
            for(let tags_name=0 ; tags_name<list_check.length ; tags_name++){
                ...
                if(list_check[tags_name] ==='Interior'){
                    this.interior_checked=true;
                    this.picklistValues.push(list_check[tags_name]);
                }
                if(list_check[tags_name] ==='Exterior'){
                    this.exterior_checked=true;
                    this.picklistValues.push(list_check[tags_name]);
                }
            }
...
//optimized version of handleselected file method
    ...
    //                 console.log('imgwatermarkmethodTime',new Date().getTime()-startTime);
    ...
    //                 var startTimeNew = new Date().getTime();
    //                 const compressedImageBuffer = await this.compressJPEG(buff, 80); // Adjust quality as needed
    //                 console.log('compressedImageBufferMethodTime',new Date().getTime()-startTimeNew);
    ...
    async uploadToAWS() {
        try {
            ...
            const uploadPromises = this.selectedFilesToUpload.map(async (file, index) => {
                ...
                let objKey = this.fileName[index].replace(/\s+/g, "_").toLowerCase() + (this.isWatermark ? '_' + this.currentDateTimeWithSeconds + '_watermark' : this.currentDateTimeWithSeconds);
                let params = {
                    Key: objKey,
    ...
    async compressJPEG(imageBuffer, quality) {
        ...
        return new Promise((resolve, reject) => {
            ...
            img.src = dataURL;
            // console.log('compressmethodTime',new Date().getTime()-startTime);
            img.onerror = (error) => {
                reject(error);
            };
        });
    ...
    tags_checked(event){
        ...
        if(event.target.name==='Interior'){
            this.interior_checked = event.target.checked;
            if(this.interior_checked){
                this.picklistValues.push(event.target.name);
        ...
        if(event.target.name==='Exterior'){
            this.exterior_checked = event.target.checked;
            if(this.exterior_checked){
                this.picklistValues.push(event.target.name);
    ...
    //             newWidth = watermarkImage.width / 2; // Adjust the width as needed
    //             newHeight = newWidth / aspectRatio;
    //         } else {// Portrait or square orientation
    //             newHeight = watermarkImage.height / 2; // Adjust the height as needed
    ...
    //         // console.log('watermarkmethodtime',new Date().getTime()-startTime);
    ...
    async imageWithWatermark(image) {
        try {
            ...
            // Calculate the aspect ratio of the file image
            const fileAspectRatio = fileImage.width / fileImage.height;
    
            // Calculate the aspect ratio of the watermark image
            const watermarkAspectRatio = watermarkImage.width / watermarkImage.height;
    
            // Calculate the dimensions of the watermark to maintain its aspect ratio
            // let newWidth, newHeight;
            // if (fileAspectRatio > watermarkAspectRatio) { // File image is wider
            // console.log('inside if-->');
            //     newWidth = fileImage.width / 2; // Adjust the width as needed
            //     newHeight = newWidth / watermarkAspectRatio;
            // } else { // File image is taller
            // console.log('inside else-->');
            //     newHeight = fileImage.height / 4; // Adjust the height as needed
            //     newWidth = newHeight * watermarkAspectRatio;
            // }
