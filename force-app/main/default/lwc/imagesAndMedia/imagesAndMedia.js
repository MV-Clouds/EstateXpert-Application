...
export default class ImagesAndMedia extends LightningElement {
  ...
  timeInString() {
    try {
      ...
      const month = (currentDateTime.getMonth() + 1)
        .toString()
        .padStart(2, "0"); // Month is zero-indexed, so add 1
        ...
        .then((result) => {
          ...
          if (result != null) {
            ...
            this.Expose = this.data
              .filter(
                (media) =>
                  media.MVEX__Sort_on_Expose__c !== null &&
                  media.MVEX__IsOnExpose__c !== false
              )
              .sort(
                (a, b) => a.MVEX__Sort_on_Expose__c - b.MVEX__Sort_on_Expose__c
              );
            this.Website = this.data
              .filter(
                (media) =>
                  media.MVEX__Sort_on_Website__c !== null &&
                  media.MVEX__IsOnWebsite__c !== false
              )
              .sort(
                (a, b) =>
                  a.MVEX__Sort_on_Website__c - b.MVEX__Sort_on_Website__c
              );
            this.Portal = this.data
              .filter(
                (media) =>
                  media.MVEX__Sort_on_Portal_Feed__c !== null &&
                  media.MVEX__IsOnPortalFeed__c !== false
              )
              .sort(
                (a, b) =>
                  a.MVEX__Sort_on_Portal_Feed__c -
                  b.MVEX__Sort_on_Portal_Feed__c
              );
            this.data.forEach(
              (row) =>
                (row.MVEX__Size__c = row.MVEX__Size__c
                  ? row.MVEX__Size__c + " " + "kb"
                  : "External")
            );
            this.data.forEach(
              (row) =>
                (row.MVEX__Tags__c = row.MVEX__Tags__c
                  ? row.MVEX__Tags__c.split(";")
                  : "")
            );
            this.isData =
              result.listingImages && result.listingImages.length > 0;
  ...
  saveChanges() {
    try {
      ...
      console.log(
        "combinedMediaListToSave-->",
        JSON.stringify(combinedMediaListToSave)
      );
      ...
      this.data.forEach((record) => {
        ...
        for (let key in record) {
          if (
            key !== "MVEX__BaseUrl__c" &&
            key !== "MVEX__Size__c" &&
            key !== "MVEX__Property__c"
          ) {
  ...
  saveOrder() {
    ...
    if (this.sortOn.includes("Expose")) {
      combinedMediaListToSave = combinedMediaListToSave.concat(
        this.prepareMediaListToSave("Expose", this.Expose)
      );
    }
    if (this.sortOn.includes("Website")) {
      combinedMediaListToSave = combinedMediaListToSave.concat(
        this.prepareMediaListToSave("Website", this.Website)
      );
    }
    if (this.sortOn.includes("Portal")) {
      combinedMediaListToSave = combinedMediaListToSave.concat(
        this.prepareMediaListToSave("Portal", this.Portal)
      );
    }
  ...
  uploadImage() {
    ...
    if (this.imageTitleToUpload && this.imageUrlToUpload) {
      ...
      if (this.selectedUrlType === "Image") {
        if (
          !this.imageUrlToUpload.match(/\.(png|jpg|jpeg|gif|png|svg)(\?.*)?$/)
        ) {
          this.showToast(
            "Error",
            "Invalid Url kindly check url and type",
            "error"
          );
          this.isPopup = true;
        } else {
      ...
      if (event.target.files.length > 0) {
        ...
        for (let file = 0; file < event.target.files.length; file++) {
          if (Math.floor(event.target.files[file].size / 1024) <= 3000) {
            ...
            this.fileSize.push(
              Math.floor(event.target.files[file].size / 1024)
            );
        ...
        if (this.largeImagefiles.length > 0) {
          this.showToast(
            "Error",
            this.largeImagefiles + " has size more than 3 mb",
            "error"
          );
        }
      }
  ...
  async uploadToAWS() {
    try {
      ...
      const uploadPromises = this.selectedFilesToUpload.map(
        async (file, index) => {
          this.showSpinner = true;
          ...
          let params = {
            ...
            Body: this.isWatermark
              ? await this.compressAndWatermarkImage(file)
              : file,
            ACL: "public-read"
          };
  ...
  async compressAndWatermarkImage(file) {
    ...
    console.log(
      "Size of buffer before compression:",
      (buff.length / 1024).toFixed(2),
      "KB"
    );
    const compressedImageBuffer = await this.compressJPEG(buff, 80); // Adjust quality as needed
    console.log(
      "Size of buffer after compression:",
  ...
  async compressJPEG(imageBuffer, quality) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const Buffer = buffer.Buffer;
        const compressedImageBuffer = new Buffer(
  ...
  handledDrop(event) {
    try {
      ...
      let reorderedMediaIds = this.reorderMediaIds(
        draggedMediaId,
        droppedMediaId,
        draggedIndex,
        droppedIndex,
  ...
  reorderMediaIds(
    ...
    if (draggedIndex < droppedIndex) {
      for (
        let i = parseInt(draggedIndex, 10);
        i < parseInt(droppedIndex, 10);
        i++
      ) {
    ...
    reorderedMediaIds[parseInt(droppedIndex, 10)] = draggedMediaId;
  ...
  async imageWithWatermark(image) {
    try {
      ...
      const resizedWatermarkImage = this.resizeImage(
        watermarkImage,
        newWidth,
        newHeight
      );
      ...
      const watermarkedImage = await watermark([
        file,
        resizedWatermarkImage
      ...
      this.showToast(
        "Error",
        "Something went wrong while adding watermark.",
        "error"
      );
      throw error;
    }
  ...
  resetCheckboxes() {
    ...
    const websiteCheckboxes =
      this.template.querySelectorAll(".checkbox-website");
